...
module.exports = {
  ...
  getFotaLogs : (req, res, next)=>{
    ...
    if(val.error){
      ...
      device.getFotaLogs(req.params?.device_id,(err,rows)=>{
        if(!err) response.send(res,rows);
        else response.error(res,httpStatus.INTERNAL_SERVER_ERROR,err);
      });
    }
  ...
  resetFotaAttempts : (req, res, next)=>{
    const val = Joi.object({
      device_id: Joi.number().required(),
    }).validate(req.params);
    ...
    if(val.error){
      ...
      device.resetFotaAttempts(req.params?.device_id,(err,rows)=>{
        if(!err) response.send(res,rows);
        else response.error(res,httpStatus.INTERNAL_SERVER_ERROR,err);
      });
    }
    ...
    try {
      ...
      const val = Joi.object({
        ...
        description: Joi.object({
          attributes: Joi.object({
            type: Joi.string()
              .valid('string', 'integer', 'float', 'boolean', 'json', 'time', 'execute', 'opaque') // Allowed values
              .required(), // 'type' is required
            title: Joi.string().required(), // 'title' is required and must be a string
            readable: Joi.boolean().required(), // 'readable' is required and must be a boolean
            writable: Joi.boolean().required(), // 'writable' is required and must be a boolean
